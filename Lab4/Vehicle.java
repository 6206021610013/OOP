...
public class Vehicle{
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println("Access class variable using dot operator: Vehicle.hasEngine=" +
        Vehicle.hasEngine);
        ...
        System.out.println("after assign object reference minivan to v, v reference the samememory block as minivan reference to");
...
/*static Vehicle passByValue(int i, Vehicle v)//methodประเภทstatic "PassByValue"มีการคืนค่ากลับเป็นชนิดobjของคลาสvehicle ส่งพารามิเตอร์2ตัวคือ int i  เปนPassbyvalue
    {                                           //่j=10, j ถูกส่งมาเป็น i i=10, sedan = v
        System.out.println("Inside method, passByValue(..)"); //print ...
        i += 1; //This will not passed back to method caller.      i จาก 10 เป็น 11
        System.out.println("Within passByValue(..): i+=1="+i);

        v.mpg +=1; //You can modify the data within the object, which will be passed back.      mpgของobj v เพิ่มขึ้น 1 ค่า ทำให้ตัวแปร mpgของobj seedan เพิ่มค่า 1 เหมือนกัน เพราะใช้งานหน่วยความจำร่วมกัน
        System.out.println("Inside passByValue(..) method, change the original object data,v.mpg+=1="+v.mpg); //ข้อมูลใน obj ต้นฉบับที่ส่งมาให้กับ obj v ก็ต้องเปลี่ยน คือ sedan.mpg เปลี่ยนค่าโดยเพิ่มขึ้นไป 1

        //Instantiate a new Vehicle object
        System.out.println("Within passByValue(..): instantiating a new object, vehicle");
        Vehicle vehicle = new Vehicle(); // สร้าง obj Vehicle ขึ้นใหม่ ชื่อว่า obj vehicle ของคลาส Vihicle
        vehicle.passengers = 5; // กำหนดตัวแปร passengers ของ obj vehicle เป็น 5 ภายใน method "passByValue"
        vehicle.mpg = 50; // กำหนดตัวแปร mpg ของ obj vehicle เป็น 50 ภายใน method "passByValue"
        vehicle.fuelcap = 20; // กำหนดตัวแปร fuelcap ของ obj vehicle เป็น 20 ภายใน method "passByValue"

        v = vehicle;//inside the method, the parameter v is now point to vehicle เอาค่าในตัวแปร obj vehicle passengers,mpg,fuelcap กำหนดค่าให้ v แบบ reference
    ...
    static Vehicle passByValue(int i, Vehicle v)//methodประเภทstatic "PassByValue"มีการคืนค่ากลับเป็นชนิดobjของคลาสvehicle ส่งพารามิเตอร์2ตัวคือ int i  เปนPassbyvalue
    {                                           //่j=10, j ถูกส่งมาเป็น i i=10, sedan = v
        System.out.println("Inside method, passByValue(..)"); //print ... ใน method passbyvalue
        i += 1; //This will not passed back to method caller.      i จาก 10 เป็น 11
        System.out.println("Within passByValue(..): i+=1="+i);

        v.mpg +=1; // mpgของobj v เพิ่มขึ้น 1 ค่า ทำให้ตัวแปร mpgของobj seedan เพิ่มค่า 1 เหมือนกัน เพราะใช้งานหน่วยความจำร่วมกัน
        System.out.println("Inside passByValue(..) method, change the original object data,v.mpg+=1="+v.mpg); //ข้อมูลใน obj ต้นฉบับที่ส่งมาให้กับ obj v ก็ต้องเปลี่ยน คือ sedan.mpg เปลี่ยนค่าโดยเพิ่มขึ้นไป 1
        ...
        v = vehicle;// เอาค่าในตัวแปร obj vehicle passengers,mpg,fuelcap กำหนดค่าให้ v เป็นแบบ reference
                    // ทำตัวแปรภายใน obj v มีค่าเหมือนกับ obj vehicle
                    // พอ obj v เปลี่ยน obj sedan ที่ถูกส่งมาเป็น obj v ก็เปลี่ยนไปด้วย passengers = 5, mpg = 50, fuelcap = 20
        System.out.println("\nInside Method passByValue(..): After v=vehicle, parameter v is pointing to new object vehicle"); //เปลี่ยนตำแหน่งไปที่หน่วยความจำของ vehicle
        System.out.println("v.passenger=vehicle.passengers="+v.passengers+"v.mpg=vehicle.mpg="+v.mpg+" v.fuelcap=vehicle.fuelcap="+v.fuelcap); // นำค่า passengers ของ obj v มาแสดง
        return vehicle; //รีเทินค่า vehicle กลับไปที่ v1
    }
    */